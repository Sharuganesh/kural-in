      ...
      <div className="relative z-10 p-5 sm:p-6">
        ...
        <div className="flex flex-wrap gap-1.5 mb-3">
          {p.tech.map((t) => (
            <span
              key={t}
              className="font-mono text-[10px] px-2 py-0.5 rounded-md border border-border text-muted-foreground"
            >
              {t}
            </span>
          ))}
  ...
  return (
    <section id="projects" className="relative py-24 sm:py-32">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        <div
          ref={headerRef}
          className={`text-center mb-12 sm:mb-16 transition-all duration-700 ${headerVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}
        >
      ...
      <div className="md:hidden relative">
        ...
        <div className="flex justify-center gap-3 mt-4">
          <button
            ...
            className="h-10 w-10 rounded-full border border-border bg-card/80 backdrop-blur text-foreground hover:bg-primary hover:text-primary-foreground transition"
          >
            ←
          </button>
          <button
